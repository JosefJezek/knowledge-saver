...
# knowledge-saver Auto-Activation Hook
# This hook ensures the knowledge-saver skill evaluates every interaction
# for extractable knowledge worth preserving.
#
# Installation:
#   1. Copy this script to ~/.claude/hooks/
#   2. Make it executable: chmod +x ~/.claude/hooks/knowledge-saver-activator.sh
...
it produced extractable knowledge using the knowledge-saver skill.
...
3. IF YES to any question above:
   ACTIVATE: Use Skill(knowledge-saver) NOW to extract the knowledge
...
The knowledge-saver skill will decide whether to actually create a new
